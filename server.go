...
func main() {
	...
	r.Get("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			l.Errorw("index page template parse fail", zap.Error(err))
			http.Error(w, `{"error": "server error"}`, http.StatusInternalServerError)
			return
		}

		if err := tmpl.ExecuteTemplate(w, "index.tmpl", nil); err != nil {
			l.Errorw("index page template execute fail", zap.Error(err))
			http.Error(w, `{"error": "server error"}`, http.StatusInternalServerError)
			return
		}
	})
	...
	r.Get("/about", func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			l.Errorw("about page template parse fail", zap.Error(err))
			http.Error(w, `{"error": "server error"}`, http.StatusInternalServerError)
			return
		}

		if err := tmpl.ExecuteTemplate(w, "about.tmpl", nil); err != nil {
			l.Errorw("about page template execute fail", zap.Error(err))
			http.Error(w, `{"error": "server error"}`, http.StatusInternalServerError)
			return
		}
	})
	...
	r.Get("/data/{page}/file.json", func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			l.Errorw("failed parsing page", zap.Error(err))
		...
		if err != nil {
			l.Errorw("failed getting page", zap.Error(err))
			http.Error(w, `{"error": "server error"}`, http.StatusInternalServerError)
			return
		}
	...
	r.Get("/stats.json", func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			l.Errorw("failed getting count", zap.Error(err))
			http.Error(w, `{"error": "server error"}`, http.StatusInternalServerError)
			return
		}
		...
		render.JSON(l, w, http.StatusOK, stats)
	})
